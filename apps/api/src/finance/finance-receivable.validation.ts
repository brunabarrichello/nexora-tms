...
export function parseUpdateCustomerReceivable(input: unknown): UpdateCustomerReceivableInput {
  ...
  if ('fiscalDocumentId' in body)
    result.fiscalDocumentId =
      optionalNullableUuid(body.fiscalDocumentId, 'fiscalDocumentId') ?? null;
  if ('fiscalReference' in body)
    result.fiscalReference = optionalText(body.fiscalReference, 'fiscalReference', 160) ?? null;
  if ('notes' in body) result.notes = optionalText(body.notes, 'notes', 1000) ?? null;
  if (Object.keys(result).length === 0)
    throw new BadRequestException('at least one receivable field must be provided');
  return result;
}
...
export function parseCancelCustomerReceivable(input: unknown): CancelCustomerReceivableInput {
  ...
  if (!reason || reason.length < 10)
    throw new BadRequestException('reason must have at least 10 characters');
  return { reason };
}

export function parseCreateCustomerReceivableTransaction(
  input: unknown,
): CreateCustomerReceivableTransactionInput {
  const body = requireObject(input);
  const kind = requireTransactionKind(body.kind);
  const relatedTransactionId =
    optionalNullableUuid(body.relatedTransactionId, 'relatedTransactionId') ?? null;
  if (kind === 'reversal' && !relatedTransactionId)
    throw new BadRequestException('reversal requires relatedTransactionId');
  if (kind === 'receipt' && relatedTransactionId)
    throw new BadRequestException('receipt cannot reference relatedTransactionId');
  return {
    kind,
...
function requireTransactionKind(value: unknown): CustomerReceivableTransactionKind {
  if (value !== 'receipt' && value !== 'reversal')
    throw new BadRequestException('kind must be receipt or reversal');
  return value;
}
