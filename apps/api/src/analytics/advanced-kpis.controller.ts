...
import {
  AdvancedKpisService,
  type AdvancedKpisQuery,
  type AdvancedKpisResult,
} from './advanced-kpis.service.js';
...
export class AdvancedKpisController {
  ...
  get(
    ...
    @Query('to') to?: string,
    @Query('comparisonFrom') comparisonFrom?: string,
    @Query('comparisonTo') comparisonTo?: string,
    @Query('customerPartyId') customerPartyId?: string,
    @Query('origin') origin?: string,
    @Query('destination') destination?: string,
    @Query('status') status?: string,
  ): Promise<AdvancedKpisResult> {
    const query: AdvancedKpisQuery = {
      from,
      to,
      comparisonFrom,
      comparisonTo,
