...
test('rejects inverted availability windows', () => {
  assert.throws(
    () =>
      parseAvailability({
        status: 'available',
        availableFrom: '2026-09-02T10:00:00Z',
        availableUntil: '2026-09-01T10:00:00Z',
      }),
    BadRequestException,
  );
});
...
test('driver and asset blocks enforce allowed severities', () => {
  assert.equal(
    parseBlock({ reasonCode: 'DOC', reason: 'Review', severity: 'compliance' }).severity,
    'compliance',
  );
  assert.equal(
    parseBlock({ reasonCode: 'MAINT', reason: 'Workshop', severity: 'maintenance' }, true).severity,
    'maintenance',
  );
  assert.throws(
    () => parseBlock({ reasonCode: 'MAINT', reason: 'Workshop', severity: 'maintenance' }),
    BadRequestException,
  );
});
...
test('asset capability temperatures must be coherent', () => {
  const value = parseAssetCapabilities({
  ...
  assert.throws(
    () => parseAssetCapabilities({ minTemperatureC: 10, maxTemperatureC: 5 }),
    BadRequestException,
  );
});
...
test('maintenance plan requires a positive time or odometer interval', () => {
  assert.equal(
    parseMaintenancePlan({ name: 'Preventive', maintenanceType: 'preventive', intervalDays: 90 })
      .intervalDays,
    90,
  );
  assert.throws(
    () => parseMaintenancePlan({ name: 'Invalid', maintenanceType: 'preventive' }),
    BadRequestException,
  );
});
...
test('maintenance monetary cost requires currency', () => {
  ...
  assert.equal(
    parseMaintenance({ maintenanceType: 'preventive', totalCost: 100, currencyId: uuid }).totalCost,
    100,
  );
});
