...
export class CapacityQualificationService {
  ...
  createDriverDocument(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, async (client, context, id) => {
      ...
      return this.insertAndFetch(
        ...
        [context.tenantId, id, data.documentTypeId, data.documentNumber, data.issuer, data.issuedOn,
          data.expiresOn, data.status, data.validationStatus, data.notes, context.userId],
        'driver_documents',
      );
    });
  }
  ...
  createDriverQualification(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, (client, context, id) =>
      this.insertAndFetch(
        ...
        [context.tenantId, id, data.qualificationType, data.code, data.name, data.certificateNumber,
          data.issuer, data.issuedOn, data.expiresOn, data.status, data.notes, context.userId],
  ...
  createDriverCourse(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, (client, context, id) =>
      this.insertAndFetch(
        ...
        [context.tenantId, id, data.courseCode, data.courseName, data.provider, data.certificateNumber,
          data.completedOn, data.expiresOn, data.workloadHours, data.status, data.notes, context.userId],
        'driver_courses',
      ),
    );
  }
  ...
  setDriverAvailability(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, async (client, context, id) => {
      const result = await client.query<{ id: string }>(
        ...
        [context.tenantId, id, data.status, data.availableFrom, data.availableUntil, data.currentCityId,
          data.destinationCityId, data.maxDistanceKm, data.notes, context.userId],
      );
  ...
  createDriverUnavailability(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, (client, context, id) =>
      this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.reasonCode,data.reason,data.startsAt,data.endsAt,data.status,context.userId],
  ...
  createDriverEmergencyContact(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, (client, context, id) =>
      this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.name,data.relationship,data.phone,data.isPrimary,data.isActive,context.userId],
  ...
  createDriverBlock(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, (client, context, id) =>
      this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.reasonCode,data.reason,data.severity,data.startsAt,data.endsAt,context.userId],
        'driver_blocks'),
    );
  }

  releaseDriverBlock(driverId: string, blockId: string, input: unknown): Promise<QualificationRecord> {
    return this.releaseBlock('driver_blocks', 'driver_id', driverId, blockId, input, this.requireDriver.bind(this));
  }
  ...
  createDriverRating(driverId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.driverCreate(driverId, async (client, context, id) => {
      if (data.transportRequestId) await this.requireTenantEntity(client, 'transport_requests', data.transportRequestId, 'transport request');
      return this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.transportRequestId,data.dimension,data.score,data.note,context.userId],
        'driver_ratings');
    });
  }
      ...
      const result = await client.query<{ id: string }>(
        `INSERT INTO capacity_asset_capabilities (
         ...
         ON CONFLICT (tenant_id,asset_id) DO UPDATE SET
           ...
           updated_by_user_id=excluded.updated_by_user_id,updated_at=now()
         RETURNING id::text AS id`,
        [context.tenantId,id,data.refrigerated,data.sealed,data.sideLoading,data.rearLoading,data.dangerousGoods,
          data.foodGrade,data.trackingCapable,data.maxPallets,data.minTemperatureC,data.maxTemperatureC,context.userId],
      );
  ...
  listAssetDocuments(assetId: string): Promise<readonly QualificationRecord[]> {
    return this.assetList('capacity_asset_documents', assetId, 'expires_on NULLS LAST, created_at DESC');
  }
  ...
  createAssetDocument(assetId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.assetCreate(assetId, async (client, context, id) => {
      ...
      return this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.documentTypeId,data.documentNumber,data.issuer,data.issuedOn,data.expiresOn,
          data.status,data.validationStatus,data.notes,context.userId], 'capacity_asset_documents');
    });
  }

  listMaintenancePlans(assetId: string): Promise<readonly QualificationRecord[]> {
    return this.assetList('capacity_asset_maintenance_plans', assetId, 'is_active DESC, next_due_on NULLS LAST, name');
  }
  ...
  createMaintenancePlan(assetId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.assetCreate(assetId, (client, context, id) =>
      this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.name,data.maintenanceType,data.intervalDays,data.intervalOdometerKm,
          data.nextDueOn,data.nextDueOdometerKm,data.isActive,data.notes,context.userId],
        'capacity_asset_maintenance_plans'),
    );
  }

  listMaintenance(assetId: string): Promise<readonly QualificationRecord[]> {
    return this.assetList('capacity_asset_maintenance', assetId, 'planned_at DESC NULLS LAST, created_at DESC');
  }

  createMaintenance(assetId: string, input: unknown): Promise<QualificationRecord> {
    const data = parseMaintenance(input);
    return this.assetCreate(assetId, async (client, context, id) => {
      if (data.maintenancePlanId) await this.requireOwnedChild(client, 'capacity_asset_maintenance_plans', data.maintenancePlanId, 'asset_id', id, 'maintenance plan');
      if (data.providerPartyId) await this.requireTenantEntity(client, 'business_parties', data.providerPartyId, 'provider party');
      return this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.maintenancePlanId,data.providerPartyId,data.maintenanceType,data.status,
          data.plannedAt,data.startedAt,data.completedAt,data.odometerKm,data.totalCost,data.currencyId,data.notes,context.userId],
        'capacity_asset_maintenance');
    });
  }

  listMaintenanceItems(assetId: string, maintenanceId: string): Promise<readonly QualificationRecord[]> {
    ...
    return this.database.withTenantContext(context, async (client) => {
      ...
      await this.requireOwnedChild(client, 'capacity_asset_maintenance', maintenance, 'asset_id', asset, 'maintenance');
      const result = await client.query<QualificationRecord>(
        `SELECT * FROM capacity_asset_maintenance_items WHERE maintenance_id=$1::uuid ORDER BY created_at,id`, [maintenance]);
      return result.rows;
    });
  }

  createMaintenanceItem(assetId: string, maintenanceId: string, input: unknown): Promise<QualificationRecord> {
    const data = parseMaintenanceItem(input);
    const asset = requireUuid(assetId, 'assetId');
    const maintenance = requireUuid(maintenanceId, 'maintenanceId');
    const context = this.tenantContext.require();
    return this.wrap(async () => this.database.withTenantContext(context, async (client) => {
      await this.requireAsset(client, asset);
      await this.requireOwnedChild(client, 'capacity_asset_maintenance', maintenance, 'asset_id', asset, 'maintenance');
      return this.insertAndFetch(client,
        ...
        [context.tenantId,maintenance,data.itemType,data.description,data.quantity,data.unitAmount,data.totalAmount,
          data.currencyId,context.userId], 'capacity_asset_maintenance_items');
    }));
  }
  ...
  createInsurance(assetId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.assetCreate(assetId, async (client, context, id) => {
      if (data.insurerPartyId) await this.requireTenantEntity(client, 'business_parties', data.insurerPartyId, 'insurer party');
      return this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.insurerPartyId,data.policyNumber,data.startsOn,data.endsOn,data.coverageAmount,
          data.currencyId,data.status,data.notes,context.userId], 'capacity_asset_insurances');
    });
  }
  ...
  createInspection(assetId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.assetCreate(assetId, (client, context, id) =>
      this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.inspectionType,data.inspectorUserId,data.performedAt,data.result,data.status,
          JSON.stringify(data.checklist),data.notes,data.nextDueAt,context.userId], 'capacity_asset_inspections'),
    );
  }
      ...
      const result = await client.query<{ id: string }>(
        `INSERT INTO capacity_asset_availability (
         ...
         ON CONFLICT (tenant_id,asset_id) DO UPDATE SET
           ...
           updated_by_user_id=excluded.updated_by_user_id,updated_at=now()
         RETURNING id::text AS id`,
        [context.tenantId,id,data.status,data.availableFrom,data.availableUntil,data.currentCityId,data.notes,context.userId],
      );
  ...
  createAssetLocation(assetId: string, input: unknown): Promise<QualificationRecord> {
    ...
    return this.assetCreate(assetId, (client, context, id) =>
      this.insertAndFetch(client,
        ...
        [context.tenantId,id,data.cityId,data.observedAt,data.latitude,data.longitude,data.source,data.accuracyM,
          data.providerReference], 'capacity_asset_locations'),
    );
  }

  listAssetBlocks(assetId: string): Promise<readonly QualificationRecord[]> {
    return this.assetList('capacity_asset_blocks', assetId, 'released_at NULLS FIRST, starts_at DESC');
  }
  ...
  releaseAssetBlock(assetId: string, blockId: string, input: unknown): Promise<QualificationRecord> {
    return this.releaseBlock('capacity_asset_blocks', 'asset_id', assetId, blockId, input, this.requireAsset.bind(this));
  }

  private async driverList(table: string, driverId: string, orderBy: string): Promise<readonly QualificationRecord[]> {
    const id = requireUuid(driverId, 'driverId');
    const context = this.tenantContext.require();
    return this.database.withTenantContext(context, async (client) => {
      await this.requireDriver(client, id);
      const result = await client.query<QualificationRecord>(`SELECT * FROM ${table} WHERE driver_id=$1::uuid ORDER BY ${orderBy}`, [id]);
      return result.rows;
    });
  }
  ...
  private async driverOne(table: string, driverId: string): Promise<QualificationRecord> {
    ...
    return this.database.withTenantContext(context, async (client) => {
      await this.requireDriver(client, id);
      const result = await client.query<QualificationRecord>(`SELECT * FROM ${table} WHERE driver_id=$1::uuid`, [id]);
  ...
  private driverCreate(
    driverId: string,
    work: (client: TenantQueryClient, context: { tenantId: string; userId: string }, driverId: string) => Promise<QualificationRecord>,
  ): Promise<QualificationRecord> {
    const id = requireUuid(driverId, 'driverId');
    const context = this.tenantContext.require();
    return this.wrap(() => this.database.withTenantContext(context, async (client) => {
      await this.requireDriver(client, id);
      return work(client, context, id);
    }));
  }

  private async assetList(table: string, assetId: string, orderBy: string): Promise<readonly QualificationRecord[]> {
    const id = requireUuid(assetId, 'assetId');
    const context = this.tenantContext.require();
    return this.database.withTenantContext(context, async (client) => {
      await this.requireAsset(client, id);
      const result = await client.query<QualificationRecord>(`SELECT * FROM ${table} WHERE asset_id=$1::uuid ORDER BY ${orderBy}`, [id]);
      return result.rows;
    });
  }
  ...
  private async assetOne(table: string, assetId: string): Promise<QualificationRecord> {
    ...
    return this.database.withTenantContext(context, async (client) => {
      await this.requireAsset(client, id);
      const result = await client.query<QualificationRecord>(`SELECT * FROM ${table} WHERE asset_id=$1::uuid`, [id]);
  ...
  private assetCreate(
    assetId: string,
    work: (client: TenantQueryClient, context: { tenantId: string; userId: string }, assetId: string) => Promise<QualificationRecord>,
  ): Promise<QualificationRecord> {
    const id = requireUuid(assetId, 'assetId');
    const context = this.tenantContext.require();
    return this.wrap(() => this.database.withTenantContext(context, async (client) => {
  ...
  private async requireTenantEntity(client: TenantQueryClient, table: string, id: string, label: string): Promise<void> {
  ...
  private async requireDocumentType(client: TenantQueryClient, id: string, scopes: readonly string[]): Promise<void> {
    const result = await client.query<{ subject_scope: string }>(
      `SELECT subject_scope FROM document_types WHERE id=$1::uuid AND is_active=true LIMIT 1`, [id]);
    const row = result.rows[0];
    if (!row) throw new NotFoundException('document type not found');
    if (!scopes.includes(row.subject_scope)) throw new BadRequestException(`document type scope ${row.subject_scope} is not compatible`);
  }
  ...
  private async requireOwnedChild(
    ...
    const result = await client.query(`SELECT 1 FROM ${table} WHERE id=$1::uuid AND ${ownerColumn}=$2::uuid LIMIT 1`, [id, ownerId]);
  ...
  private async fetchById(client: TenantQueryClient, table: string, id: string): Promise<QualificationRecord> {
    const result = await client.query<QualificationRecord>(`SELECT * FROM ${table} WHERE id=$1::uuid`, [id]);
  ...
  private async wrap<T>(work: () => Promise<T>): Promise<T> {
    try {
      ...
      if (code === '23505') throw new ConflictException('qualification record conflicts with an existing active record');
      if (code === '23503') throw new BadRequestException('referenced entity does not exist in this tenant');
      if (code === '23514' || code === '22P02') throw new BadRequestException('qualification data violates a database constraint');
      throw error;
    }
  }
}
