...
export function parseCompliancePolicy(input: unknown): DocumentCompliancePolicyInput {
  ...
  return {
    documentTypeId: requireUuid(stringValue(body.documentTypeId, 'documentTypeId', 80), 'documentTypeId'),
    requiredForContracting,
    requiredForTrip,
...
export function parseComplianceOverride(input: unknown): DocumentComplianceOverrideInput {
  ...
  if (Number.isNaN(parsedUntil)) throw new BadRequestException('validUntil must be an ISO-8601 date-time');
  const now = Date.now();
  if (parsedUntil <= now) throw new BadRequestException('validUntil must be in the future');
