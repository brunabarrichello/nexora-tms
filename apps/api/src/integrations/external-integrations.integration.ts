...
try {
  ...
  try {
    ...
    await assert.rejects(
      () =>
        secretProbe.query('select secret_hash from integration_clients where id=$1::uuid', [
          clientA,
        ]),
      /permission denied/i,
    );
  ...
  await worker.query('select nexora_worker_record_webhook_attempt($1::uuid,2,$2,503,14,$3,true)', [
    deliveryId,
    'failure',
    'fixture HTTP 503 terminal',
  ]);
  ...
  await worker.query('select nexora_worker_record_webhook_attempt($1::uuid,3,$2,204,9,null,true)', [
    deliveryId,
    'success',
  ]);
  ...
  const ownerVisibleAudit = await app
    .query<{ action: string; total: string }>(
