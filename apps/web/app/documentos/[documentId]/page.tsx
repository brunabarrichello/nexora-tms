...
export default async function Page({ params, searchParams }: Readonly<{
  ...
  if (document.kind !== 'ready') {
    return <OperationalPage eyebrow="Documents • Wave 0018" title="Documento" description="Detalhe documental tenant-aware." status="Indisponível" filters={[]} columns={[{ key: 'status', label: 'Estado' }]} emptyTitle="Não foi possível carregar o documento" emptyDescription={document.message} actions={[{ href: '/documentos', label: 'Voltar', variant: 'secondary' }]} />;
  }
  ...
  return (
    <OperationalPage
      ...
      description={[documentText(d.document_type_name), d.external_reference ? `Ref. ${documentText(d.external_reference)}` : null, `Escopo ${documentText(d.subject_scope)}`].filter(Boolean).join(' • ')}
      status={values.saved === '1' ? 'Operação concluída' : 'API conectada'}
      metrics={[
        { label: 'Status', value: documentStatusLabel(effectiveStatus(d)), helper: 'Lifecycle efetivo.' },
        { label: 'Validade', value: documentDate(d.expires_on), helper: d.has_expiry === true ? 'Obrigatória para este tipo.' : 'Opcional para este tipo.' },
        { label: 'Versões', value: String(versionItems.length), helper: 'Arquivos confirmados.' },
        { label: 'Validações', value: String(validationItems.length), helper: d.requires_validation === true ? 'Tipo exige validação.' : 'Validação não obrigatória.' },
      ]}
      filters={[]}
      columns={view === 'versions' ? [
        { key: 'version', label: 'Versão' }, { key: 'file', label: 'Arquivo' }, { key: 'mime', label: 'MIME' }, { key: 'size', label: 'Tamanho' }, { key: 'source', label: 'Origem' }, { key: 'created', label: 'Criada em' },
      ] : [
        { key: 'type', label: 'Tipo' }, { key: 'result', label: 'Resultado' }, { key: 'version', label: 'Versão' }, { key: 'provider', label: 'Referência' }, { key: 'validated', label: 'Validada em' },
      ]}
      rows={rows}
      totalRows={rows.length}
      tabs={[{ href: `/documentos/${documentId}?view=versions`, label: 'Versões' }, { href: `/documentos/${documentId}?view=validations`, label: 'Validações' }]}
      actions={[
      ...
      emptyTitle={view === 'versions' ? 'Nenhuma versão registrada' : 'Nenhuma validação registrada'}
      emptyDescription={view === 'versions' ? 'Nenhum arquivo foi confirmado pelo storage.' : 'Nenhuma decisão foi adicionada ao histórico.'}
      integrationNotes={[
