...
export async function RecordEditorPage({
  ...
  if (!config)
    return (
      <EditorState title="Recurso inválido" message="O cadastro solicitado não está disponível." />
    );
  if (mode === 'edit' && !config.supportsEdit) {
    return (
      <EditorState
        title="Edição indisponível"
        message="O contrato atual deste recurso oferece apenas criação e consulta."
        backHref={config.returnPath}
      />
    );
  }
  ...
  if (!id)
    return (
      <EditorState
        ...
        message="O identificador do registro é obrigatório."
        backHref={config.returnPath}
      />
    );
...
function initialValues(
  ...
  for (const field of fields) {
    ...
    if (Array.isArray(value))
      values[field] = value.filter((entry): entry is string => typeof entry === 'string');
